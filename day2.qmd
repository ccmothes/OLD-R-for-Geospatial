...
Yesterday you were exposed to spatial data types and various databases you can pull spatial data from. Today we are going to use those data sets to perform a range of spatial analyses.
...
We have to start by reading in the packages we need for today. Some are repeats from yesterday, but we also have a couple new ones. Namely `ggplot2` and `tmap` to do some quick visualizations of our spatial outputs. Tomorrow we will be using these packages for more advanced data visualizations.
...
We're going to start off today with some distance calculations. Using our species occurrence data, say we want to compare each species average distance to rivers and roads.
...
Lets read in our land cover and elevation raster files. These are files you downloaded yesterday, but were already processed. You can read in a raster file with the `rast()` function from `terra`.
...
Looks like elk are associated with the most forested habitats. We can also view the spread of values with a boxplot.
...
This works similar to what we ran with the occurrence dataset...however we notice that it returns the raw raster values instead of the names land cover classes (which we want). This raster behaves such that values are named factors, and we can pull this metadata with the `cats()` function.
...
We can now get some summary statistics on the most common land cover types
