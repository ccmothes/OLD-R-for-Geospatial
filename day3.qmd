...
First we need to call in the packages needed for today's lesson. Load in our `packageLoad()` function that we have now saved as a script. You can run other scripts using the `source()` function.

```{r}
source("packageLoad.R")

packageLoad(c("tmap", "ggplot2", "shiny"))
```
